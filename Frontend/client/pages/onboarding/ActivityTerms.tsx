...
const ActivityTerms = () => {
  ...
  return (
    <div className="onboarding-layout min-h-screen flex flex-col bg-white">
      {/* Header */}
      <div className="flex h-14 w-full items-center justify-start px-6 lg:px-16 border-b border-ds-pebble bg-white">
        <LogoWebsite />
      </div>
        ...
        <div className="flex items-start justify-between w-full max-w-5xl gap-16">
          ...
          <div className="flex-1 max-w-xl flex flex-col gap-8">
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-sm text-ds-slate">
                By proceeding with the verification process on{" "}
                <span className="font-semibold text-ds-charcoal">Travel Homes</span>, you agree to
                the following:
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {terms.map((term) => (
                <div
                  key={term.title}
                  className="flex flex-col gap-0.5 p-4 rounded-2xl border border-ds-pebble bg-ds-linen/40"
                >
                  <p className="text-sm font-semibold text-ds-charcoal">{term.title}</p>
                  <p className="text-sm text-ds-slate">{term.body}</p>
                </div>
              ))}
            </div>
          ...
          <div className="hidden lg:flex flex-1 max-w-md items-center justify-center">
            <div className="relative">
              <div className="absolute inset-0 rounded-3xl bg-ds-linen scale-110 blur-2xl opacity-60" />
              <img
                src="/placeholder.svg"
