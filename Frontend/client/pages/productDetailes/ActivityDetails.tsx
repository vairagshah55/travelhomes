...
export default function UniqueStayDetails() {
  ...
  const [checkInDate, setCheckInDate] = useState(new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }));
  const [checkOutDate, setCheckOutDate] = useState(new Date(Date.now() + 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }));
        ...
        <Button
          onClick={() => {
            if (selectedRangeState.start && selectedRangeState.end) {
              const startStr = selectedRangeState.start.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
              const endStr = selectedRangeState.end.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
              onApplyRange(startStr, endStr);
              onClose();
            }
          }}
