...
export default function BlogDetailsPage() {
  ...
  // Determine slug from route params or query string. The URL is read at
  // every render but it's cheap (no allocation in the common case).
  const slug = (() => {
    const url = new URL(window.location.href);
    return slugFromParams || url.searchParams.get("slug") || undefined;
  })();
      ...
      <div className="min-h-screen flex-col flex gap-0 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-200 transition-colors">
        ...
        <div className="min-h-screen mt-2 bg-white text-gray-900 font-sans pb-12">
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-0 mt-4">
            ...
            {error && <div className="text-red-600">{error.message || "Failed to load blog"}</div>}
          </div>
            ...
            <>
              ...
              <div className="bg-black py-2 px-4 md:px-24 w-full">
                <div className="flex mt-6 md:mt-10 items-center gap-3 text-gray-200 mb-4">
                  {blog.authorImg ? (
                    <img
                      src={getImageUrl(blog.authorImg) || "/user-avatar.svg"}
                  ...
                  <div>
                    ...
                    {blog.authorRole && <p className="text-[10px] opacity-60">{blog.authorRole}</p>}
                  </div>
                </div>
                <h1 className="text-2xl md:text-3xl font-bold text-gray-50 mb-3">{blog.title}</h1>
              </div>
