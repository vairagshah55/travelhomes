 ...
 const [loadingConversations, setLoadingConversations] = useState(false);
  ...
  useEffect(() => {
    const fetchChatProfile = async () => {
      ...
      try {
        ...
        console.log("res of fetch profile:", res)

        const json = await res.json();
        if (json.success) {
           console.log("Chat profile loaded:", json.data);
  ...
  useEffect(() => {
    const handleReceiveMessage = (data: any) => {
        ...
        setChats(prevChats => {
            ...
            // Move to top
            const targetChatIdx = updatedChats.findIndex(c => c.id === data.chatId);
            if (targetChatIdx > -1) {
                const [targetChat] = updatedChats.splice(targetChatIdx, 1);
  ...
  useEffect(() => {
    const fetchMissingConversation = async (convId: string) => {
        try {
            ...
            if (json.success) {
                const c = json.data;
                const other = c.participants.find((p: any) => (p.refId._id || p.refId) !== chatProfile?.id);
                const otherUser = other?.refId || {};
    ...
    if (convId && chats.length > 0) {
      ...
      if (found) {
          ...
          // If not found in current list, fetch it from backend
        ...
        {loadingConversations ? (
          <div className="flex flex-col items-center justify-center h-full p-4">
            ...
            <div className="flex flex-col items-center justify-center h-full p-4">
                ...
                <p className="text-gray-400 text-sm text-center">Contact a vendor from their service details page to start a chat.</p>
            </div>
        ) : (
          chats
            .filter(
  ...
  return (
    <>
      ...
      {/* Responsive Layout */}
      <div className="dark:dark-color h-full bg-white overflow-x-hidden pt-[64px] max-lg:pt-0 md:pt-[85px]">
        <div className="font-sans h-[calc(100vh-0px)] md:h-[calc(100vh-112px)] flex">
          {isMobile ? (
