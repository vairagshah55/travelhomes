...
type BlogDTO = { _id: string; title: string; slug: string; coverImage?: string; createdAt?: string };
...
const DEFAULT_TESTIMONIALS: PublicTestimonial[] = [
  { id: "t1", userName: "Aarav Mehta",  rating: 5, content: "An amazing experience! The booking process was seamless, and the support team was super helpful throughout.", avatar: "/test5.png",       email: "aarav.mehta@example.com",  createdAt: "2025-10-12T09:24:00Z" },
  { id: "t2", userName: "Priya Sharma", rating: 4, content: "Really impressed with how easy it was to find what I needed. The platform feels modern and intuitive.",          avatar: "/user-avatar.svg", email: "priya.sharma@example.com", createdAt: "2025-09-28T14:45:00Z" },
  { id: "t3", userName: "Rahul Verma",  rating: 5, content: "Exceptional service and great user experience. I found exactly what I was looking for within minutes!",         avatar: "/test2.png",       email: "rahul.verma@example.com",  createdAt: "2025-08-20T11:10:00Z" },
  { id: "t4", userName: "Neha Kapoor",  rating: 5, content: "Loved the design and smooth navigation. It's rare to see a platform this well thought out!",                    avatar: "/test3.png",       email: "neha.kapoor@example.com",  createdAt: "2025-07-05T18:20:00Z" },
  { id: "t5", userName: "Rohan Singh",  rating: 4, content: "Great experience overall! Would definitely recommend this to anyone looking for reliable service.",              avatar: "/test4.png",       email: "rohan.singh@example.com",  createdAt: "2025-06-15T07:40:00Z" },
];

const SECTION_DEFAULTS: Record<string, boolean> = {
  "camper-van": true, "unique-stays": true, "best-activity": true,
  "trending-destinations": true, testimonials: true, "top-rated-stays": true, faq: true,
};
...
export default function Index() {
  ...
  const { data: categoryMapData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const add = (list: any[], type: string) => { if (Array.isArray(list)) list.forEach((item) => { if (item.name) map[item.name.toLowerCase()] = type; }); };
      add(van, "caravan"); add(stay, "unique-stays"); add(act, "activity");
      return map;
    },
    staleTime: 10 * 60 * 1000,
  });

  const { data: offersResponse, isLoading: loadingOffers, isError: isOfferError } = useQuery({
  ...
  useEffect(() => {
    const filterMap: Record<string, string> = { "camper-van": "camper-van", "unique-stays": "unique-stays", activity: "best-activity" };
    const tabMap:    Record<string, string> = { "unique-stays": "unique-stays", activities: "best-activity", caravan: "camper-van" };
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const sectionMap: Record<string, FilterType> = { "camper-van": "camper-van", "unique-stays": "unique-stays", activity: "activity" };
  ...
  const getNormCategory = (cat?: string, serviceType?: string) => {
    ...
    if (["caravan","campervan","campertrailer","motorhome","rv","van"].some((k) => cc.includes(k))) return "caravan" as const;
    if (cc.includes("stay") || ["uniquestays","unique","stays","glamping","resort","villa"].includes(cc)) return "unique-stays" as const;
  ...
  const mapOfferToCard = (o: OfferDTO): CardItem => {
    ...
    const img   = o.photos?.coverUrl?.length ? o.photos.coverUrl : o.photos?.galleryUrls?.[0] || "/placeholder.svg";
    return {
      id: route, title: o.name,
      ...
      unit, image: img,
      images: [...(o.photos?.coverUrl ? [o.photos.coverUrl] : []), ...(Array.isArray(o.photos?.galleryUrls) ? o.photos.galleryUrls : [])].filter(Boolean).slice(0, 5),
    };
  };

  const approved      = offers.filter((o) => o.status === "approved");
  const caravanCards  = useMemo(() => approved.filter((o) => getNormCategory(o.category, o.serviceType) === "caravan").map(mapOfferToCard),  [approved, categoryMap]);
  const stayCards     = useMemo(() => approved.filter((o) => getNormCategory(o.category, o.serviceType) === "unique-stays").map(mapOfferToCard), [approved, categoryMap]);
  const activityCards = useMemo(() => approved.filter((o) => getNormCategory(o.category, o.serviceType) === "activity").map(mapOfferToCard), [approved, categoryMap]);
  ...
  const incPage = (k: "caravan" | "unique-stays" | "activity") =>
    setPages((p) => ({ ...p, [k]: Math.min(k === "caravan" ? caravanTotal : k === "unique-stays" ? stayTotal : activityTotal, p[k] + 1) }));
    ...
    <>
      ...
      <div ref={contentRef} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-6">
        ...
        {homepageSections["top-rated-stays"] && (
          <Section title="Top Rated Stays" subtitle="Consistently loved by guests across India" className="py-8 md:py-12">
            {loadingOffers ? (
              ...
              <>
                ...
                {stayCards.length > stayShown.length && (
                  <div className="mt-6 flex justify-center">
                    <Button onClick={() => incPage("unique-stays")} className="rounded-full bg-blue-600 hover:bg-blue-700 text-white px-8 h-11 shadow-sm transition-all duration-200 hover:shadow-md active:scale-[0.98]">
                      View more
                    </Button>
                  </div>
                )}
