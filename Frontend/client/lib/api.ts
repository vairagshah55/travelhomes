...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  const headers = {
    ...
    "Content-Type":
      options && options.headers && (options.headers as any)["Content-Type"]
...
export const authApi = {
  register: (payload: RegisterPayload) =>
    request<{ success: boolean; userType: "user" | "vendor"; data: any; registerId: string }>(
      `/api/auth/register`,
      {
        method: "POST",
  ...
  updateRegister: (id: string, payload: Partial<RegisterPayload>) =>
    request<{ success: boolean; data: any }>(`/api/auth/register/${id}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  verifyRegisterOtp: (id: string, otp: string) =>
    request<{ success: boolean }>(`/api/auth/register/${id}/verify-otp`, {
      method: "POST",
      body: JSON.stringify({ otp }),
    }),
  resendRegisterOtp: (id: string) =>
    request<{ success: boolean }>(`/api/auth/register/${id}/resend-otp`, {
      method: "POST",
    }),
  googleAuth: (code: string) =>
    request<{ success: boolean; token: string; user: any }>(`/api/auth/google`, {
      method: "POST",
...
export const vendorAuthApi = {
  login: (payload: {
    ...
    }>(`/api/vendorlogin/login`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  forgot: (payload: { email: string }) =>
    request<{ success: boolean; message: string }>(`/api/vendorlogin/forgot`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  verifyOtp: (payload: { email: string; otp: string }) =>
    request<{ success: boolean; message: string }>(`/api/vendorlogin/verify-otp`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  reset: (payload: { email: string; token: string; newPassword: string }) =>
    request<{ success: boolean; message: string }>(`/api/vendorlogin/reset`, {
      method: "POST",
      body: JSON.stringify({
        email: payload.email,
        otp: payload.token,
  ...
  updateAccount: (payload: {
    ...
    request<{ success: boolean; message: string; user: any }>(`/api/vendorlogin/update-account`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  sendChangeOtp: (payload: {
    currentEmail: string;
    userType?: string;
    newEmail?: string;
    newMobile?: string;
  }) =>
    request<{ success: boolean; message: string; otp?: string }>(
      `/api/vendorlogin/send-change-otp`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      },
...
export const vendorSettingApi = {
  get: (vendorId: string) =>
    request<{ success: boolean; data: VendorSettingDTO }>(`/api/vendorsetting/${vendorId}`),
  create: (payload: VendorSettingDTO) =>
    request<{ success: boolean; data: VendorSettingDTO }>(`/api/vendorsetting`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  upsert: (vendorId: string, payload: Partial<VendorSettingDTO>) =>
    request<{ success: boolean; data: VendorSettingDTO }>(`/api/vendorsetting/${vendorId}`, {
      method: "PUT",
      body: JSON.stringify(payload),
    }),
  updateSection: (vendorId: string, section: "general" | "account" | "preferences", payload: any) =>
    request<{ success: boolean; data: VendorSettingDTO }>(
      `/api/vendorsetting/${vendorId}/${section}`,
...
export const bookingDetailsApi = {
  ...
  remove: (code: string, token?: string) =>
    request<{ success: boolean; message: string }>(`/api/bookingDetails/${code}`, {
      method: "DELETE",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }),
  invoice: (code: string, token?: string) =>
    request<{ success: boolean; data: { invoiceUrl: string; invoiceData: any; printData: any } }>(
      `/api/bookingDetails/${code}/invoice`,
      {
        method: "POST",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      },
...
export const offersApi = {
  ...
  get: (id: string, token?: string) =>
    request<ApiItemResponse<OfferDTO>>(`/api/offers/${id}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }),
  create: (payload: Partial<OfferDTO>, token?: string) =>
    request<ApiItemResponse<OfferDTO>>("/api/offers", {
      method: "POST",
      body: JSON.stringify(payload),
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }),
  update: (id: string, payload: Partial<OfferDTO>, token?: string) =>
    request<ApiItemResponse<OfferDTO>>(`/api/offers/${id}`, {
      method: "PUT",
      body: JSON.stringify(payload),
      headers: token ? { Authorization: `Bearer ${token}` } : {},
  ...
  remove: (id: string, token?: string) =>
    request<{ success: boolean; message: string }>(`/api/offers/${id}`, {
      method: "DELETE",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }),
...
export const vendorAnalyticsApi = {
  getCounts: (token?: string) =>
    request<{ success: boolean; data: VendorAnalyticsCounts }>(`/api/vendorAnalytics`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }),
  getGraphs: (token?: string, period: string = "monthly") =>
    request<{ success: boolean; data: VendorAnalyticsGraphData[] }>(
      `/api/vendorAnalytics/graphs?period=${period}`,
      {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      },
    ),
...
export interface UserProfileDTO {
  ...
  vendorStatus?:
    ...
    | "inactive"
    | "banned"
    | "kyc-unverified"; // Added vendorStatus
  phoneNumber?: string;
...
export const userProfileApi = {
  get: (email: string) =>
    request<{ success: boolean; data: UserProfileDTO }>(
      `/api/profile?email=${encodeURIComponent(email)}`,
    ),
  upsert: (payload: Partial<UserProfileDTO> & { email: string }) =>
    request<{ success: boolean; data: UserProfileDTO }>(`/api/profile`, {
      method: "PUT",
      body: JSON.stringify(payload),
    }),
...
export const activitiesApi = {
  ...
  myList: (token: string) =>
    request<{ success: boolean; data: ActivityDTO[] }>(`/api/activities/vendor/mine/list`, {
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    }),
  // Vendor: create
  create: (token: string, payload: Partial<ActivityDTO>) =>
    request<{ success: boolean; id: string; data: ActivityDTO }>(`/api/activities`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify(payload),
    }),
  // Vendor: update
  update: (token: string, id: string, payload: Partial<ActivityDTO>) =>
    request<{ success: boolean; data: ActivityDTO }>(`/api/activities/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify(payload),
    }),
  // Vendor/Admin: set status
  setStatus: (token: string, id: string, status: "draft" | "published" | "archived") =>
    request<{ success: boolean; data: ActivityDTO }>(`/api/activities/${id}/status`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ status }),
    }),
...
export const cmsPublicApi = {
  ...
  getFeatures: async (category: string, type?: string) => {
    ...
    const res = await request<{ success: boolean; data: any[] }>(
      `/api/cms/features?${params.toString()}`,
    );
    return res.data || [];
  },
};
...
export const bookingsApi = {
  getUserBookings: (userId: string, token: string) =>
    request<{ success: boolean; bookings: BookingDTO[] }>(`/api/bookings/user/${userId}`, {
      headers: { Authorization: `Bearer ${token}` },
    }),
  getBookingById: (id: string, token: string) =>
    request<{ success: boolean; booking: BookingDTO }>(`/api/bookings/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    }),
  create: (payload: Partial<BookingDTO>, token?: string) =>
    request<{ success: boolean; booking: BookingDTO }>("/api/bookings", {
      method: "POST",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      body: JSON.stringify(payload),
    }),
  updateStatus: (id: string, status: string, token: string) =>
    request<{ success: boolean; booking: BookingDTO }>(`/api/bookings/${id}/status`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ status }),
    }),
  ...
  generateInvoice: (id: string, token: string) =>
    request<{ success: boolean; invoice: any }>(`/api/bookings/${id}/invoice`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    }),
};
...
export const settingsApi = {
  getSeo: (page: string) =>
    request<{ success: boolean; data: any }>(`/api/settings/seo?page=${page}`),
};
...
export const cmsPagesApi = {
  getPage: (key: string) =>
    request<{
      success: boolean;
      data: { title: string; content: string; sections?: { heading: string; content: string }[] };
    }>(`/api/cms/pages/${key}`),
};
...
export const notificationsApi = {
  list: (unreadOnly?: boolean, limit: number = 50, recipientRole: string = "vendor") => {
    ...
    return request<{ success: boolean; data: NotificationDTO[]; totalUnread: number }>(
