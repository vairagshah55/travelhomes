...
export const ChatBox: React.FC<ChatBoxProps> = ({
  ...
  const unreadCount = messagesList.filter((m) =>
    userType == "user1" ? m.user1read : m.user2read,
  ).length;
            ...
            <>
              <div className="w-full flex justify-between items-center p-4 border-b bg-white">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-3">
                    {isMobile && (
                      <button className="lg:hidden text-black dark:text-white px-4">
              ...
              >
                {chatMessages.map((group, idx) => (
                  <div
                    key={idx}
                    className={`relative flex items-end gap-2 group ${group.me ? "justify-end" : "justify-start"}`}
                  >
                    {!group.me && (
                      <img
                    ...
                    <div
                      className={`max-w-[350px] h-full p-3 px-5 rounded-[10px] ${
                        group.me ? "bg-black text-white" : "bg-[#f3f4f6] text-black"
                      }`}
                    >
                      {/* Attachments Grid */}
                      {group.attachments && group.attachments.length > 0 && (
                        <div
                          className={`grid gap-1 mb-2 ${group.attachments.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}
                        >
