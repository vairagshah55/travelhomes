...
const getFeaturesForProperty = (propertyType: string, category: string, stayType: string) => {
  const baseFeatures = [
    { label: "Washroom", value: "washroom", icon: Bath },
    { label: "Hot Water", value: "hot_water", icon: Flame },
    { label: "Fan", value: "fan", icon: Fan },
    { label: "AC", value: "ac", icon: Wind },
    { label: "Drinking Water", value: "drinking_water", icon: Droplets },
  ...
  // Features for individual rooms
    ...
    const roomFeatures = [
      ...
      { label: "Mini Fridge", value: "mini_fridge", icon: Refrigerator },
      { label: "Safe", value: "safe", icon: "🔒" },
      { label: "Work Desk", value: "desk", icon: Table },
      { label: "Room Service", value: "room_service", icon: "🍽️" },
      { label: "Sea View", value: "sea_view", icon: "🌊" },
      { label: "Mountain View", value: "mountain_view", icon: "🏔️" },
    ];
...
useEffect(() => {
      ...
      try {
        ...
        if (data && data.type === 'stay' && data.doc && ['pending', 'draft', 'rejected'].includes(data.doc.status)) {
          ...
          setTermsAccepted(false);
           ...
           // Check if user has any approved stays to prevent re-auto-fill (only auto-fill once/first time)
           let hasApprovedStays = false;
           if (user?.id) {
             try {
                const res = await offersApi.list("approved");
                if (res.success && res.data) {
                   hasApprovedStays = res.data.some((offer: any) => offer.vendorId === user.id);
                }
             } catch (e) {
                console.error("Failed to check approved stays", e);
             }
           }
  ...
  const handleComplete = async () => {
    try {
      ...
      roomsWithPhotos[0] = { ...roomsWithPhotos[0], photos: stayType === "entire" ? entireStayGallery : gallery };
      ...
      <div className="flex-1 w-full overflow-y-auto pt-14 pb-28">
        <div className="max-w-7xl mx-auto w-full px-6 lg:px-20 flex flex-col items-center gap-10 py-8">
          ...
          <div className="flex flex-col items-center gap-8 w-full max-w-4xl">
              ...
              {/* Row 1 */}
              <div className="max-w-4xl mx-auto flex items-start gap-3 flex-wrap justify-center">
                {propertyTypes.map((property) => (
                  <div
                    key={property.id}
                    onClick={() => togglePropertySelection(property.id)}
                    className={`flex flex-col w-[208px] p-2 max-sm:p-[9px_10px] items-center max-sm:flex-row justify-center gap-2 rounded-xl border cursor-pointer transition-all duration-200 ${
                  ...
                  >
                    <p>
                      <img
                        ...
                        alt={property.name}
                        className="w-[25px] h-[25px] aspect-square object-contain"
                        onError={(e) => { e.currentTarget.style.display = 'none'; }}
                      />
                    </p>
                    <p className="text-xl font-medium text-[#4B4B4B] dark:text-gray-300 font-plus-jakarta">
                      {property.name}
                    </p>
                  </div>
                ))}
                    ...
                    <div key={propertyId} className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-wrap gap-3 justify-center">
                        {categories.map((category) => (
                          <div
                            key={`${propertyId}-${category.id}`}
                            onClick={() => {
                              setSelectedCategories((prev) => {
                                const categoryKey = `${propertyId}-${category.id}`;
                                const isSelected = prev.includes(categoryKey);
                                if (isSelected) {
                                  return prev.filter((id) => id !== categoryKey);
                                } else {
                                  return [...prev, categoryKey];
                                }
                              });
                            }}
                            className={`flex w-[180px] p-[12px_16px] flex-col justify-center items-center gap-2 rounded-xl border cursor-pointer transition-all duration-200 ${
                              selectedCategories.includes(`${propertyId}-${category.id}`)
                                ? "border-[#131313] bg-[#F8F9FA] dark:border-white dark:bg-gray-700"
                                : "border-[#EAECF0] bg-white dark:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600"
                            }`}
                          >
                            <img src={getImageUrl(category.icon)} className="w-8 h-8 mb-2" alt={category.name} />
                            <p className="text-sm font-medium text-[#4B4B4B] dark:text-gray-300 font-plus-jakarta text-center">
                              {category.name}
                            </p>
                          </div>
                        ))}
                ...
                {stayType === "entire" && (
                  <>
                    ...
                    <div className="flex flex-col gap-5">
                      ...
                      <label className="flex h-12  flex-col items-center justify-center w-full p-6 text-sm bg-[#F9FAFB] dark:bg-gray-700 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-gray-400 transition-colors cursor-pointer">
                ...
                <div className="flex gap-5">
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    <input
                      ...
                      maxLength={50}
                      className="w-full h-[50px] px-3 py-4 border border-[#EAECF0] dark:border-gray-600 rounded-lg text-sm text-black dark:text-gray-300 font-['Plus_Jakarta_Sans'] bg-white dark:bg-gray-800 focus:outline-none focus:border-[#131313]"
                    />
                    {errors.brandName && (
                      <span className="text-red-500 text-xs mt-1">
                        {errors.brandName}
                ...
                <div className="flex gap-5 max-md:flex-wrap">
                  ...
                  <div className="flex flex-col flex-1">
                    <input
                      ...
                      maxLength={6}
                             inputMode="numeric"
                      className="flex-1 h-[50px] px-3 py-4 border border-[#EAECF0] dark:border-gray-600 rounded-lg text-sm text-black dark:text-gray-300 font-['Plus_Jakarta_Sans'] bg-white dark:bg-gray-800 focus:outline-none focus:border-[#131313]"
                    />
          ...
          <div className="flex flex-col items-center gap-8 w-full max-w-4xl">
              ...
              <div className="w-full max-w-4xl mx-auto flex flex-col gap-5">
                  ...
                  <div className="flex max-md:flex-wrap gap-5">
                      ...
                      <div className="w-full relative">
                        <select
                          ...
                          className="w-full h-[50px] px-3 py-4 border border-[#EAECF0] dark:border-gray-600 rounded-lg text-sm text-black dark:text-gray-300 font-['Plus_Jakarta_Sans'] bg-white dark:bg-gray-800 focus:outline-none focus:border-[#131313] appearance-none"
                        >
                          <option value="" disabled>
                            Select State
                          </option>
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div className="w-[242px] h-[175px] flex flex-col items-center justify-center gap-4 bg-[#F9FAFB] dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-600 relative overflow-hidden">
                    {idProofImage ? (
                      <img 
                        src={idProofImage} 
                        alt="ID Proof" 
...
{currentStep === 7 && (
  <div className="fixed flex max-md:flex-col max-md:gap-3 overflow-hidden">
    ...
    <div className="flex justify-center items-center">
      ...
      {/* Footer */}
      <div className="flex w-full z-30 fixed bottom-0 items-center justify-center px-5 lg:px-20 py-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 shadow-lg">
  <div className="max-w-4xl mx-auto w-full flex justify-between items-center gap-5">
    ...
    {currentStep !== 7 && (
      <div className="flex items-center max-md:flex-col gap-6">
        <span className="text-lg font-semibold text-[#131313] dark:text-white">
          {currentStep + 1}/{totalSteps - 1} Completed
        </span>
    ...
    {currentStep === 7 ? (
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={handleBack}
          className="flex items-center gap-2 px-8 py-[14px] rounded-[60px] border border-gray-200 dark:border-gray-600"
        >
          <span>Back</span>
        </button>
        ...
        <button
          onClick={handleNext}
          className="flex items-center justify-center px-8 py-[14px] bg-[#131313] text-white rounded-[60px]"
        >
          <span>Next</span>
        </button>
      </div>
