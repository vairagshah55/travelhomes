...
const registerUserOrVendor = async (req, res) => {
  try {
    ...
    if (userType === "user" && !mobile) {
      return res
        .status(400)
        .json({ success: false, message: "Mobile number is required for user registration." });
    }
    ...
    let verifiedRegistration = await Register.findOne({
    ...
    if (userType === "user") {
      ...
      created = await User.findOneAndUpdate({ email: normalizedEmail }, payload, {
        upsert: true,
        new: true,
      });
    } else {
      ...
      created = await Vendor.findOneAndUpdate({ email: normalizedEmail }, payload, {
        upsert: true,
        new: true,
      });
    }
...
const verifyRegisterOtp = async (req, res) => {
  try {
    ...
    if (!id || !otp) return res.status(400).json({ success: false, message: "Missing id or otp." });
    ...
    if (process.env.LOG_OTP_DEBUG === "true") {
      console.log(
        `[VERIFY OTP DEBUG] registerId=${id} storedOtp="${storedOtp}" providedOtp="${providedOtp}" match=${storedOtp === providedOtp}`,
      );
    }
...
const resendRegisterOtp = async (req, res) => {
  try {
    ...
    if (process.env.LOG_OTP_DEBUG === "true") {
      console.log(
        `[RESEND OTP] email=${doc.email} otp=${otp} (type: ${typeof otp}) expiresAt=${otpExpiresAt.toISOString()}`,
      );
    }
