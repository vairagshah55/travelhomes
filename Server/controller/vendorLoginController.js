  ...
  try {
    ...
    if (docs.length === 0) {
      if (process.env.LOG_AUTH_DEBUG === "true") {
        console.log("[LOGIN] Not found record", {
          email: isEmail ? inputVal : undefined,
          mobile: !isEmail ? inputVal : undefined,
        });
      }
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (docs.length === 0) {
      ...
      if (userExists) {
        console.log(
          "[RESET DEBUG] User exists, but OTP mismatch or expired. Stored OTP:",
          userExists.otp,
          "Expiry:",
...
const updateAccount = async (req, res) => {
  try {
    ...
    if (currentPassword) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Current password is required to update password" });
    }
    ...
    if (email && (doc.userType === "vendor" || !doc.userType)) {
      // Defaults to vendor often
      const Vendor = require("../models/Vendor");
      await Vendor.updateMany(
        { email: lowerCurrentEmail },
  ...
  try {
    ...
    if (newMobile) {
      try {
        ...
        return res.status(500).json({
          success: false,
          message: "Failed to send OTP to new mobile: " + smsError.message,
        });
      }
    }
