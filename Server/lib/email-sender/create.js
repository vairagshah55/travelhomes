...
const getImage = async (doc, invoice) => {
  const res = await fetch(invoice.company_info.logo, { encoding);
...
const generateHeader = (doc, invoice) => {
  ...
  doc
    ...
    .fontSize(12)
    .font('Helvetica-Bold')
    .text(invoice?.company_info?.company, 200, 50, { align)
    .fontSize(10)
    .font('Helvetica')
    .text(invoice?.company_info?.address, 200, 65, { align)
    .text(invoice?.company_info?.phone, 200, 80, { align)
    .text(invoice?.company_info?.email, 200, 95, { align)
    .text(invoice?.company_info?.website, 200, 108, { align)
    .moveDown();
};
...
function generateCustomerInformation(doc, invoice) {
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .text('Invoice To', 200, 140, { align)
    .font('Helvetica')
    .fontSize(10)
    .text(invoice.user_info.name, 200, 155, { align)
    .text(invoice.user_info.email, 200, 170, { align)
    .text(invoice?.user_info?.phone, 200, 200, { align)
    .text(invoice?.user_info?.address, 200, 185, { align);
...
function generateFooter(doc) {
  doc
    .fontSize(15)
    .text('Thanks for your order', 50, 750, { align);
}
...
function generateTableRow(
  ...
  doc
    ...
    .text(unitCost, 280, y, { width)
    .text(quantity, 370, y, { width)
    .text(lineTotal, 0, y, { align);
}
