...
function DefaultCard({ CardData, activeFilter }: { CardData: any[]; activeFilter: FilterType }) {
  ...
  return (
    <div className="overflow-x-auto scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
      <div
        className={`flex md:grid md:grid-cols-2 ${location.pathname.includes("/search") ? "lg:grid-cols-3" : "lg:grid-cols-4"} gap-5`}
      >
        {CardData.map((item) => {
            ...
            >
                ...
                >
                  <Heart
                    className={`w-[26px] h-[26px] transition-all duration-200 ${
                      isLiked
                        ? "fill-[#128086] text-[#128086] drop-shadow-[0_1px_2px_rgba(0,0,0,0.25)]"
              ...
              <div className="pt-3 space-y-0.5">
                ...
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-semibold text-[15px] leading-tight text-[#0a1c1c] line-clamp-1">
                    {item.title}
                  </h3>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <StarIcon className="w-[14px] h-[14px] fill-current text-[#0a1c1c]" />
                    <span className="text-[14px] text-[#0a1c1c]">4.91</span>
                  </div>
                </div>

                {/* Location */}
                <p className="text-[14px] text-[#717171] truncate">{item.details}</p>

                {/* Price */}
                <p className="text-[14px] text-[#0a1c1c] pt-1.5">
                  {item.Maxprice && (
                    <span className="text-[#717171] line-through mr-1.5">₹{item.Maxprice}</span>
                  )}
