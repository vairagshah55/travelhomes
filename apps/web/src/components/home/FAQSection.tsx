...
interface FAQSectionProps {
  ...
  faqs: PublicFaq[];
  /** True while the FAQ query is still in flight — drives the skeleton. */
  isLoading: boolean;
  activeTab: string;
...
export function FAQSection({
  ...
  // Nothing to show once loading has settled — no FAQs at all, or none that
  // belong to a service that is still switched on. Drop the whole block
  // rather than rendering an empty heading with dead tabs.
  const hasFaqsToShow = visibleFAQTabs.some((tab) => filteredFaqs(tab.id).length > 0);
  if (!hasFaqsToShow) return null;
