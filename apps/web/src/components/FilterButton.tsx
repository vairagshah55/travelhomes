  ...
  return (
    <motion.button
      ...
      className={`relative flex items-center gap-2 md:gap-2.5 px-5 py-2.5 rounded-full transition-colors duration-200 border overflow-hidden ${
        active
          ? "bg-[#0F5C8A] text-white border-[#0F5C8A] shadow-[0_4px_16px_rgba(15,92,138,0.35)]"
          : isHero
          ? "bg-white/10 backdrop-blur-md text-white border-white/20 hover:bg-white/20 hover:border-white/40"
          : "bg-white text-[#0A2B40] border-[#E4E8F0] hover:border-[#0F5C8A] hover:bg-[#E8F2F8] shadow-sm"
      }`}
    >
      {active && (
        <motion.span
      ...
      <span className="relative z-10 text-sm font-semibold capitalize tracking-tight">
        {label}
      </span>
    </motion.button>
  );
