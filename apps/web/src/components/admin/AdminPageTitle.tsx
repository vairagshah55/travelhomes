      ...
      <div className="min-w-0">
        {isNested && (
          ...
          >
            <Link
              to="/admin/dashboard"
              className="flex items-center justify-center w-5 h-5 rounded hover:text-[#0F5C8A] transition-colors"
            ...
            {crumbs.slice(0, -1).map((crumb) => (
              <React.Fragment key={crumb.href}>
                ...
                <Link
                  to={crumb.href}
                  className="px-1 rounded hover:text-[#0F5C8A] transition-colors truncate max-w-[160px]"
                >
                  {crumb.label}
                </Link>
              </React.Fragment>
