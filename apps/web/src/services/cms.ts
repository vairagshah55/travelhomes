...
export const cmsService = {
  ...
  updateFAQ: async (
    id: string,
    payload: { category?: string; question?: string; answer?: string },
  ) => {
  ...
  // No status param => every blog, drafts included. The admin table needs both.
  ...
  updatePage: async (
    key: string,
    payload: { title: string; content?: string; sections?: any[]; isActive?: boolean },
  ) => {
  ...
  // NOTE: the server mounts these as /read/:id and /reply/:id (not /:id/read).
  markContactRead: async (id: string) => {
    const res = await api.patch(`/admin/contact/read/${id}`);
  ...
  replyToContact: async (id: string, payload: { subject: string; body: string }) => {
    const res = await api.post(`/admin/contact/reply/${id}`, payload);
    return res.data;
  },
