...
import { FiArrowLeft, FiCopy, FiTrash2, FiX } from "react-icons/fi";
...
const ChatPage = () => {
    ...
    const fetchChatProfile = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/vendorchats/profile?email=${currentUser.email}&type=${type}`,
        );
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/vendorchats/conversations?participantKind=${chatProfile.type}&participantId=${chatProfile.id}`,
      );
      const json = await res.json();
      if (json.success) {
        const mappedChats = json.data.map((c: any) => {
          const other = c.participants.find(
            (p: any) => p.refId._id !== chatProfile.id && p.refId !== chatProfile.id,
          );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setChats((prev) =>
        prev.map((chat) => ({ ...chat, lastSeen: formatRelativeTime(chat.lastMessageTime) })),
      );
    }, 60000);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        setLoadingChat(true);
        const res = await fetch(
          `${API_BASE}/api/vendorchats/conversations/${selectedUser.id}/messages`,
        );
        const json = await res.json();
        if (json.success) {
          const mapped = (json.data || []).map((m: any) => ({
            ...
            sender:
              m.senderId?.name ||
              m.senderId?.brandName ||
              (m.senderKind === "Vendor" ? "Vendor" : "You"),
            text: m.content || "",
          ...
          setChats((prev) =>
            prev.map((c) => (c.id === selectedUser.id ? { ...c, unreadCount: 0 } : c)),
          );
        }
      } catch (e) {
  ...
  useEffect(() => {
    const handleReceiveMessage = (data: any) => {
      if (!chats.find((c) => c.id === data.chatId)) {
        fetchConversations();
        return;
      }
      setChats((prevChats) => {
        const updated = prevChats.map((c) => {
          ...
          return {
            ...
            unreadCount: selectedUser?.id === data.chatId ? 0 : c.unreadCount + 1,
          };
        });
  ...
  // Deep-link: auto-select conversation from router state, fetching it if not yet in list
  useEffect(() => {
    const fetchMissingConversation = async (convId: string) => {
      try {
        const res = await fetch(`${API_BASE}/api/vendorchats/conversations/${convId}`);
        const json = await res.json();
        if (json.success) {
          const c = json.data;
          const other = c.participants.find(
            (p: any) => (p.refId._id || p.refId) !== chatProfile?.id,
          );
          const otherUser = other?.refId || {};
          const newChat = {
            id: c._id,
        ...
        console.error("Failed to fetch missing conversation", e);
      }
    };

    const convId = location.state?.conversationId;
    if (!convId || !chats.length) return;
    const found = chats.find((c) => c.id === convId);
    if (found) {
      if (selectedUser?.id !== convId) {
        selectUser(found);
        window.history.replaceState({}, document.title);
      }
    } else {
      fetchMissingConversation(convId);
    }
  }, [chats, location.state, selectedUser?.id, chatProfile, API_BASE]);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        ...
        if (uploadJson.success) uploadedAttachments = uploadJson.data;
      }
      ...
      const recipient = selectedUser.participants?.find(
        (p: any) => (p.refId?._id || p.refId) !== chatProfile?.id,
      );
      socket.emit("send_message", {
        ...
        content: newMessage.text,
        attachments: uploadedAttachments,
        recipientId: recipient?.refId?._id || recipient?.refId,
      });
    } catch (e) {
  ...
  const handleLongPressStart = (idx) => {
    const timer = setTimeout(() => setSelectedMessageIdx(idx), 500);
  ...
  const handleDeleteMessage = async (index) => {
    ...
    if (!msgToDelete.id) {
      setSelectedUser({
        ...selectedUser,
        messages: selectedUser.messages.filter((_, i) => i !== index),
      });
  ...
  const emptyStateText = isVendor
    ? "Your conversations with users will appear here once they contact you."
    : "Contact a vendor from their service details page to start a chat.";
        ...
        <div className="flex-1 overflow-y-auto pt-6 scrollbar-hide">
          {loadingConversations ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-4">
              ...
              <p className="text-gray-400 text-sm text-center">{emptyStateText}</p>
            </div>
          ) : (
            chats
              .filter(
                (u) => u.type === filter && u.name.toLowerCase().includes(searchTerm.toLowerCase()),
              )
              .sort((a, b) => {
              ...
              .map((u) => (
                <div
                  ...
                  } ${pinnedUserIds.includes(u.id) ? "border-blue-400 border-2" : "lg:border-y"}`}
                >
                  <div className="flex items-center gap-4 w-full">
                    ...
                    <div className="flex justify-between w-full">
                      <div>
                        <p
                          className={`font-medium text-sm ${u.unreadCount > 0 ? "text-black font-bold" : ""}`}
                        >
                          {u.name}
                        </p>
                        <p
                          className={`text-xs ${u.unreadCount > 0 ? "text-black font-bold" : "text-gray-500"}`}
                        >
                          {u.preview?.length > 35 ? u.preview.slice(0, 35) + "..." : u.preview}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span
                          className={`text-[10px] ${u.unreadCount > 0 ? "text-blue-600 font-bold" : "text-gray-500"}`}
                        >
                          {u.lastSeen}
                        </span>
                        {u.unreadCount > 0 && (
          ...
          >
            ...
            {selectedUser.messages.map((msg, idx) => (
              ...
              >
                ...
                <div
                  className={`max-w-[350px] h-full p-3 px-5 rounded-[10px] ${msg.me ? "bg-[#1E3A8A] text-white" : "bg-[#f3f4f6] text-black"}`}
                >
                  {msg.attachments && msg.attachments.length > 0 && (
                    <div
                      className={`grid gap-1 mb-2 ${msg.attachments.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}
                    >
                  ...
                  <div
                    className={`text-[10px] mt-1 flex justify-end ${msg.me ? "text-gray-300" : "text-gray-500"}`}
                  >
                    <span>{msg.time}</span>
                  </div>
                </div>
                ...
                <div
                  className={`absolute -bottom-8 ${msg.me ? "right-10" : "left-10"} ${selectedMessageIdx === idx ? "flex" : "hidden"} gap-1 bg-white shadow-sm border p-1 rounded-[10px] z-10`}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <button
                ...
                className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
                  messageText.trim() || selectedFiles.length > 0
                    ? "bg-[#1E3A8A] text-white hover:bg-[#2D4DA8] shadow-lg transform active:scale-95"
