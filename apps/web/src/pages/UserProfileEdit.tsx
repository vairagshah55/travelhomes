...
const EditProfileHeader = ({ onSave, saving }: EditProfileHeaderProps) => (
  <div className="hidden md:flex flex-col md:flex-row justify-between items-start md:items-center mb-9">
    ...
    <Button
      onClick={onSave}
      disabled={saving}
      className="bg-black hover:bg-gray-800 text-white px-6 rounded-full font-geist disabled:opacity-60"
    >
      {saving ? "Saving…" : "Save"}
    </Button>
  </div>
  ...
  <div className="space-y-8">
    ...
    <div className="lg:hidden flex items-start w-full mb-9 mt-6">
      <Button
        onClick={onSave}
        disabled={saving}
        className="bg-black hover:bg-gray-800 dark:bg-black dark:text-white text-white px-8 py-6 rounded-full w-full font-geist disabled:opacity-60"
      >
        {saving ? "Saving…" : "Save"}
      </Button>
    </div>
