...
const Career = () => {
  ...
  const { data: jobs = staticJobs } = useQuery<Job[]>({
    queryKey: ["cms", "jobs", "public"],
    queryFn: async () => {
      try {
      ...
      <section className="px-4 md:px-10 py-4 dark:bg-black dark:text-white">
        <div className=" max-w-7xl mx-auto items-cente flex flex-col dark:bg-black dark:text-white dark:border-slate-300 md:flex-row md:items-center md:justify-between border rounded-xl p-6 gap-4 shadow-sm bg-gray-50">
          <div className="flex-1">
            {jobs.length > 0 ? (
              <>
                <h2 className="text-xl font-semibold dark:bg-black dark:text-white">
                  {jobs[0].position}
                </h2>
              ...
              <>
                ...
                <p className="text-sm text-gray-700 dark:bg-black dark:text-white mt-2">
                  We’ll post open roles here. Check back soon!
                </p>
              </>
            )}
          </div>
          {jobs[0] && (
            <button
              onClick={() => handleOpenDetails(jobs[0])}
              className="flex items-center border border-gray-400 rounded-full text-sm py-2 px-4 hover:bg-gray-100 dark:hover:bg-slate-600  transition whitespace-nowrap"
            >
                ...
                >
                  ...
                  <div className="flex gap-2 mt-3 md:mt-0">
                    <button
                      ...
                      className="px-5 py-2 bg-white dark:bg-black dark:text-white text-black text-sm rounded-full border border-gray-400 hover:bg-[#128086] hover:text-white transition flex items-center"
                    >
                      View More <SlArrowDown className="ml-2 h-3 w-3" />
                    </button>
                    <button
            ...
            {values.map((value, index) => (
              <div
                ...
                className={`p-6 rounded-xl shadow-md text-center border dark:border-slate-300 transition ${
                  index === 0 || index === 3 ? "bg-[#117479] text-white" : "bg-white text-gray-700"
                }`}
              >
          ...
          <div className="bg-white dark:bg-black dark:text-white rounded-xl max-w-2xl w-full relative">
            <div className="max-h-[85vh] overflow-y-auto p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-base">
                      {selectedJobForDetails.experience || "Not specified"}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              ...
              <div className="mt-8 flex justify-end gap-4">
                <button
                  onClick={() => setShowDetailsModal(false)}
                  className="px-6 py-2 border border-gray-300 rounded-full text-sm font-medium hover:bg-gray-50 dark:hover:bg-[#128086] transition"
                >
                  Close
                </button>
                <button
