...
export default function Header({ variant = "white", className = "" }: HeaderProps) {
  ...
  // The Admin app's `/dashboard` is for admin staff, not a "switch to vendor"
  // mode — there's no vendor surface here. Just navigate to the dashboard.
  ...
  return (
    <nav
      className={`flex items-center justify-between px-4 md:px-20 py-4 ${
        isTransparent ? "bg-transparent" : "bg-white shadow-sm border-b border-gray-200"
      } ${className}`}
    >
      {/* Logo */}
